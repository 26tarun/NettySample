...
public class HeartbeatManager extends Thread {
	...
	private Management generateHB() {
		...
		jb.setNameSpace(""); //hardcoded
		jb.setOwnerId(0); //hardcoded
		jb.setBid(jobId); //hardcoded
		jb.setJobId(String.valueOf(jobId));
		b.setJobBid(jb.build());
		logger.debug("**********setting Queue Size**********: "+jobId);
		//
