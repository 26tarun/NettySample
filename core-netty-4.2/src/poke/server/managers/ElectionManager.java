...
import poke.server.storage.jdbc.ClusterMapperManager;
import poke.server.storage.jdbc.ClusterMapperStorage;
import poke.server.storage.jdbc.DbConfigurations;
...
public class ElectionManager implements ElectionListener {
	...
	public static ClusterDBServiceImplementation ci = new ClusterDBServiceImplementation();
	...
	public void processRequest(Management mgmt) {
		...
		if (req.getAction().getNumber() == LeaderElection.ElectAction.DECLAREELECTION_VALUE)
			...
			logger.debug("***************This is the bomb********************");
			if(this.leaderNode == null) {
				logger.debug("***************This is the bomb but no leader********************");
	...
	public void assessCurrentState(Management mgmt) {
		...
		logger.debug("The leader that has been elected is:" + this.leaderNode);
	...
	public void concludeWith(boolean success, Integer leaderID) {
		if (success) {
			...
			// Kartik changes
			// Fetching the leader address information.
			// Database code to update leader list in database.
			if(conf.getNodeId() == leaderID) {
				ClusterMapperStorage cs = null;
				if(ci != null) {
					try {
						logger.debug("Inserting leader node: " + DbConfigurations.getIp() + conf.getPort());
						cs = new ClusterMapperStorage(DbConfigurations.getIp() , conf.getPort());
						ci.createMapperStorage(cs);
						//ClusterMapperManager.setClusterDetails(nd.getHost(), nd.getPort());
			...
			synchronized (syncPt) {
				...
				try {
					...
					if (election instanceof FloodMaxElection) {
						logger.warn("Node " + conf.getNodeId() + " setting max hops to arbitrary value (3)");
						((FloodMaxElection) election).setMaxHops(3);
					}
