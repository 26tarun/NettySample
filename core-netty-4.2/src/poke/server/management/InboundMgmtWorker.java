	...
	public void run() {
		while (true) {
			...
			try {
				...
				if(mgmt.hasJobBid()) {
					logger.debug("In Has hasJobBid");
					logger.debug("Queue size" + mgmt.getJobBid().getBid());
					logger.debug(""+mgmt.getHeader().getOriginator());
					AntConstants.getInstance().setLocationDetails(mgmt.getHeader().getOriginator(), mgmt.getJobBid().getBid());
				}
				...
				if (mgmt.hasBeat()) {
					...
					logger.debug("In Has election" + mgmt.getElection().getElectId() + "  " + mgmt.getElection().getDesc());
					logger.debug(""+mgmt.getElection().getAction());
					ElectionManager.getInstance().processRequest(mgmt);
				} else if (mgmt.hasGraph()) {
					logger.debug("In Has hasGraph");
					NetworkManager.getInstance().processRequest(mgmt, msg.channel);
				} else if (mgmt.hasJobBid()) {
					logger.debug("In Has hasJobBid");
					logger.debug("Queue size" + mgmt.getJobBid());
					//JobManager.getInstance().processRequest(mgmt);
				} else if (mgmt.hasJobPropose()) {
					logger.debug("In Has hasJobPropose");
					JobManager.getInstance().processRequest(mgmt);
				} else
